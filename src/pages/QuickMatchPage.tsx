...
export default function QuickMatchPage() {
  const { gameState, resources, updateResources, dispatchGameState } = useGame();

  const [selectedPlayerId, setSelectedPlayerId] = useState<string | null>(null);
  const [isSimulatingMatch, setIsSimulatingMatch] = useState(false);
  const [matchResult, setMatchResult] = useState<any>(null);
  const canAfford = useMemo<boolean>(() => (resources.coins >= QUICK_MATCH_FEE.coins && resources.shuttlecocks >= QUICK_MATCH_FEE.shuttlecocks), [resources]);
    ...
    setTimeout(() => {
      ...
      if (player) {
        try {
          ...
          if (result.newInjury) {
            dispatchGameState({ type: 'ADD_INJURY', payload: {playerId: player.id, injury: result.newInjury} });
            recordInjuriesChange(player, [...(player.injuries || []), result.newInjury]);
          }
