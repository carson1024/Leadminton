...
export default function TournamentCard({ 
  ...
  const timeLeft = useCountdown(tournament.startDate);
    ...
    <div className="bg-white rounded-xl shadow-md p-6">
      <div className="flex justify-between items-start mb-4">
        <div>
          ...
          <span className={`inline-block px-2 py-1 rounded-full text-sm ${tierColor} mt-1`}>
            {tierLabel}
          </span>
        </div>
        <div className="flex items-center space-x-2">
          ...
          <span className={`px-3 py-1 rounded-full text-sm ${
            tournament.status === 'upcoming' ? 'bg-green-100 text-green-800' :
      ...
      <div className="space-y-4">
        <div className="grid grid-cols-2 gap-4">
          <div>
            <h4 className="text-sm font-medium text-gray-600 mb-1">Entry Fee</h4>
            <div className="space-y-1">
              {Object.entries(tournament.entryFee).map(([resource, amount]) => (
                amount > 0 && (
          ...
          <div>
            <h4 className="text-sm font-medium text-gray-600 mb-1">Prize Pool</h4>
            <div className="space-y-1">
              {Object.entries(tournament.prizePool).map(([place, rewards]) => (
                <div key={place} className="text-sm">
                  {place}: {Object.entries(rewards).map(([resource, amount]) => 
                    `${amount} ${resource}`
                  ).join(', ')}
                </div>
              ))}
